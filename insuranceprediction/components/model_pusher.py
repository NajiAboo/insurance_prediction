...
from insuranceprediction.entity.config_entity import ModelPusherConfig
from insuranceprediction.exception import InsurancePredictionException
from insuranceprediction.entity.artifact_entity import ModelPusherArtifact, ModelEvaluationArtifact

class ModelPusher:
    def __init__(self, model_pusher_config: ModelPusherConfig, model_eval_artifact: ModelEvaluationArtifact) -> None:
        try:
            logging.info(f" {'>>'*30} Model Pusher started  {'<<'*30}")     
    ...
    def export_model(self)-> ModelPusherArtifact:
        try:
            ...
            logging.info(f"Exporing model file: {export_model_file_path}")
